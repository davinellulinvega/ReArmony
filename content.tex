...
\section{Results}
%% TODO: Rephrase this introduction
%% TODO: What programming language and libraries did you use for your implementation?
...
\fancysummary{%
The study presented by~\citet{Armony1995} and reproduced in this chapter has two purposes. The first is to validate the three following principles:
\begin{enumerate}
   \item Processing units: populations of real cells coding for the same piece of information can be abstracted as non-linear summing devices;
   \item Dual pathway connectivity: sensory information describing the state of the environment is encoded by two parallel systems, before converging on the amygdala. This is a model suggested by~\citet{Ledoux1992}, as well as~\citet{Romanski1992};
   \item Learning: the synaptic strength for the connections between neurons is updated using a modified Hebbian learning rule also known as the Stent-Hebb rule~\supercite{stent_physiological_1973}.
\end{enumerate}
Secondly, it attempts to show that computational models could be a worthwhile tool to validate mechanisms suggested by neuroscience and then inform future endeavors by making predictions. To test the validity of their three principles,~\citet{Armony1995} built an anatomically constrained neural network, and tested it using a simulated fear conditioning experiment. Through this simulation they evaluated whether the network would exhibit behavioral responses characteristic of conditioned animals. At the physiological level, they examined whether the neural units making up the network would develop `\emph{receptive fields}' similar to those observed in single-cell recordings. Since sound was used as the CS in this experiment, the term receptive field is defined as the frequency range for which a neural unit produces a non-zero output. The outcomes of my re-creation of this experiment match very closely the results published by~\citet{Armony1995}. As both sets of findings were reproduced by the neural network. It follows that the three principles guiding the model's design are valid. More importantly though, this project constitutes further evidence that computational models ``\emph{offer a complementary approach to experimental studies}''~\citet[p.~252]{Armony1995}. Finally, the model of the two-pathways to the amygdala has been integrated in the design of the \textbf{PrimEmo} architecture (see Chapter~\ref{chap:primEmo} as well as Appendix~\ref{app:primEmo}). Furthermore, even though \textbf{ProtoEmo} lacks any cortical area (see Chapter~\ref{chap:protoemo} and Appendix~\ref{app:protoEmo} for a description of \textbf{ProtoEmo}'s design), the model suggested by~\citet{Ledoux1992} and validated by this study helps predict the encoding and, therefore, detecting capabilities of the amygdala.}
\fi
